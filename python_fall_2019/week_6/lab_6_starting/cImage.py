# (Complicated) Demo for Lab 6

# Modified to download image from web, then invert pixel colors,
#   finally saving both original modified image as .gif within test_graphics folder
#
# To run this, you'll need to install the extra package Pillow.  You can do this in PyCharm,
#   or else using Anaconda's console running conda...
...
if __name__ == '__main__':
    ...
    link = "http://www.ericlevel.com/moxie_replica.JPG"
    f = urllib.request.urlopen(link)
    myfile = f.read()
    outFile = open("test_graphics/moxie_replica.JPG", "wb")
    ...
    win = ImageWin("My Window", 1536, 2048)
    oImage = FileImage('test_graphics/moxie_replica.JPG')

    print(oImage.getWidth(), oImage.getHeight())
    oImage.draw(win)
    myImage = oImage.copy()
    print("now inverting image pixels, row by row...")
    numRows = myImage.getHeight()
    for row in range(myImage.getHeight()):
        if row % 100 == 0: print("row", row, "/", numRows)
    ...
    print("you clicked on: ", win.getMouse())
    myImage.save('test_graphics/moxie-inverted.gif')
    # print(myImage.toList())
    print("Done! Check image in test_graphics folder.")
    win.exitOnClick()
